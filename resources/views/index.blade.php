                ...
                <div class="grid gap-6 md:grid-cols-2">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="kategori_resiko" name="kategori_resiko"
                                ...
                                @foreach ($kategori_resiko as $kategori)
                                    <option value="{{ $kategori->id }}" @if (old('kategori_resiko') == $kategori->id) selected
                                    @endif>
                                        {{ $kategori->nama }}
                                    </option>
                                @endforeach
                            ...
                            <select id="sub_kategori" name="sub_kategori"
                                ...
                                @if (old('kategori_resiko'))
                                    @foreach ($sub_kategori as $subKategori)
                                        <option value="{{ $subKategori->id }}" @if (old('sub_kategori') == $subKategori->id) selected
                                        @endif>{{ $subKategori->nama }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="kelompok_resiko" name="kelompok_resiko"
                                ...
                                @foreach ($kelompok_resiko as $kelompok)
                                    <option value="{{ $kelompok->id }}" @if (old('kelompok_resiko') == $kelompok->id) selected
                                    @endif>
                                        {{ $kelompok->nama }}
                                    </option>
                                @endforeach
                            ...
                            <select id="kejadian_resiko" name="kejadian_resiko"
                                ...
                                @if (old('kelompok_resiko'))
                                    @foreach ($kejadian_resiko_filtered as $kejadian)
                                        <option value="{{ $kejadian->id }}" @if (old('kejadian_resiko') == $kejadian->id) selected
                                        @endif>{{ $kejadian->nama }}
                                        </option>
                                    @endforeach
                                @endif
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="w-full mx-auto max-w-md">
                            <form id="risikoForm">
                                <div class="mb-4">
                                    <label for="tipe_sumber_risiko" class="block text-sm font-medium text-gray-700">Tipe
                                        Sumber Risiko</label>
                                    <select id="tipe_sumber_risiko" name="tipe_sumber_risiko"
                                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                        <option value="">Pilih Tipe Sumber Risiko</option>
                                ...
                                <div class="mb-4">
                                    <label for="area_dampak" class="block mb-2 font-bold text-gray-700">Area
                                        Dampak</label>
                    ...
                    <script>
                        $(document).ready(function () {
                            $('#tipe_sumber_risiko').change(function () {
                                var tipeSumberRisiko = $(this).val();
                                var penyebabDropdown = $('#penyebab');

                                // Kosongkan dropdown penyebab
                                penyebabDropdown.empty();
                                penyebabDropdown.append('<option value="">Pilih Penyebab</option>');

                                // Tambahkan pilihan berdasarkan tipe sumber risiko
                                if (tipeSumberRisiko === 'Internal') {
                                    penyebabDropdown.append('<option value="Sumber Daya Manusia">Sumber Daya Manusia</option>');
                                    penyebabDropdown.append('<option value="Kondisi Keuangan Sehat">Kondisi Keuangan Sehat</option>');
                                    penyebabDropdown.append('<option value="Lingkungan Pembangkit">Lingkungan Pembangkit</option>');
                                    penyebabDropdown.append('<option value="Man">Man</option>');
                                    penyebabDropdown.append('<option value="Money">Money</option>');
                                    penyebabDropdown.append('<option value="Enviromental">Enviromental</option>');
                                    penyebabDropdown.append('<option value="Machine">Machine</option>');
                                    penyebabDropdown.append('<option value="Kinerja Proyek">Kinerja Proyek</option>');
                                    penyebabDropdown.append('<option value="Sumber Daya Alam">Sumber Daya Alam</option>');
                                    penyebabDropdown.append('<option value="Material(Bahan Bakar)">Material(Bahan Bakar)</option>');
                                    penyebabDropdown.append('<option value="Method(Manajemen, Tata Kelola)">Method(Manajemen, Tata Kelola)</option>');
                                    penyebabDropdown.append('<option value="Peraturan Perusahaan">Peraturan Perusahaan</option>');
                                    penyebabDropdown.append('<option value="Operari Dan Pemeliharaan">Operari Dan Pemeliharaan</option>');
                                    penyebabDropdown.append('<option value="Kesehatan Dan Keselamatan Kerja">Kesehatan Dan Keselamatan Kerja</option>');
                                } else if (tipeSumberRisiko === 'External') {
                                    penyebabDropdown.append('<option value="Alam">Alarm</option>');
                                    penyebabDropdown.append('<option value="Keamanan">Keamanan</option>');
                                    penyebabDropdown.append('<option value="Ekonomi">Ekonomi</option>');
                                    penyebabDropdown.append('<option value="Politik/Sosial/Budaya">Politik/Sosial/Budaya</option>');
                                    penyebabDropdown.append('<option value="Pasar">Pasar</option>');
                                    penyebabDropdown.append('<option value="Teknologi">Teknologi</option>');
                                    penyebabDropdown.append('<option value="Hukim/Peraturan">Hukim/Peraturan</option>');
                                }
                            });
                        });
                        function openModal() {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (data) {
                                            ...
                                            $.each(data, function (key, value) {
                                                $('#sub_kategori').append('<option value="' + value.id + '">' + value.nama + '</option>');
                                            });
                                        }
                                    });
                                } else {
                                ...
                                if (kelompokId) {
                                    $.ajax({
                                        ...
                                        success: function (data) {
                                            ...
                                            $.each(data, function (key, value) {
                                                $('#kejadian_resiko').append('<option value="' + value.id + '">' + value.deskripsi + '</option>');
                                            });
                                        },
