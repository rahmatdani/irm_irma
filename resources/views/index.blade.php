...
<body class="min-h-screen p-4 bg-gray-100">
    <div class="w-full mx-auto max-w-7xl">
        ...
        <div class="p-6 mb-6 bg-white rounded-lg shadow-md">
            <div class="flex items-center justify-between">
                ...
                <button onclick="openModal()" class="px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-600 focus:outline-none">
                    Tambah Resiko
                </button>
            </div>
        </div>
    ...
    <!-- Modal -->
    <div id="formModal" class="fixed inset-0 hidden w-full h-full overflow-y-auto bg-gray-600 bg-opacity-50">
        <div class="relative w-full max-w-6xl p-5 mx-auto bg-white border rounded-md shadow-lg top-20">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div class="space-y-4">
                        <div class="mb-4">
                            ...
                            <select id="kategori_resiko" name="kategori_resiko" class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline">
                                <option value="">Pilih Kategori Resiko</option>
                                @foreach($kategori_resiko as $kategori)
                                    <option value="{{ $kategori->id }}" @if(old('kategori_resiko') == $kategori->id) selected @endif>{{ $kategori->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="sub_kategori" name="sub_kategori" class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline">
                                <option value="">Pilih Sub Kategori</option>
                                @if (old('kategori_resiko'))
                                    @foreach($sub_kategori as $subKategori)
                                        <option value="{{ $subKategori->id }}" @if(old('sub_kategori') == $subKategori->id) selected @endif>{{ $subKategori->nama }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="kelompok_resiko" name="kelompok_resiko" class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline">
                                <option value="">Pilih Kelompok Resiko</option>
                                @foreach($kelompok_resiko as $kelompok)
                                    <option value="{{ $kelompok->id }}" @if(old('kelompok_resiko') == $kelompok->id) selected @endif>{{ $kelompok->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="kejadian_resiko" name="kejadian_resiko" class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline">
                                <option value="">Pilih Kejadian Resiko</option>
                                @if (old('kelompok_resiko'))
                                    @foreach($kejadian_resiko_filtered as $kejadian)
                                        <option value="{{ $kejadian->id }}" @if(old('kejadian_resiko') == $kejadian->id) selected @endif>{{ $kejadian->nama }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                    ...
                    <div class="space-y-4">
                        <div class="mb-4">
                            <label for="tipe_sumber_risiko" class="block mb-2 font-bold text-gray-700">Tipe Sumber Risiko</label>
                            <input type="text" id="tipe_sumber_risiko" name="tipe_sumber_risiko" class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline" placeholder="Masukkan Tipe Sumber Risiko">
                        </div>

                        <div class="mb-4">
                            <label for="penyebab" class="block mb-2 font-bold text-gray-700">Penyebab</label>
                            <textarea id="penyebab" name="penyebab" class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline" rows="3" placeholder="Masukkan Penyebab"></textarea>
                        </div>

                        <div class="mb-4">
                            <label for="area_dampak" class="block mb-2 font-bold text-gray-700">Area Dampak</label>
                            <input type="text" id="area_dampak" name="area_dampak" class="w-full px-3 py-2 leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline" placeholder="Masukkan Area Dampak">
                        </div>
                    </div>
                </div>

                <div class="mt-6 text-center">
                    <button type="submit" class="px-6 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-700 focus:outline-none focus:shadow-outline">Simpan</button>
                </div>
            </form>
        </div>
    </div>
